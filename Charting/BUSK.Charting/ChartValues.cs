...
//#endif
        ...
        public PointTracker GetTracker(ISeriesView view)
        {
            ...
            if (Trackers.TryGetValue(view, out PointTracker tracker)) return tracker;
        ...
        private IPointEvaluator<T> GetConfig(ISeriesView view)
        {
            ...
            //series == null means that chart values are null, and BUSK.Charting
            //could not set the Series Instance tho the current chart values...
            if (view == null || view.Model.SeriesCollection == null) return null;


            if ((view.Configuration ?? view.Model.SeriesCollection.Configuration) is IPointEvaluator<T> config) return config;
